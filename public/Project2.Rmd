...
library(knitr)
opts_chunk$set(fig.align="center", fig.height=5, message=FALSE, warning=F, fig.width=8,tidy.opts=list(width.cutoff=60),tidy=TRUE)
```
...
#SST <- sum(dist^2)/77
SST <- sum(dist^2)/77
SSW <- cereal1 %>% group_by(mfr) %>% select(sodium, rating, calories) %>% do(d=dist(.[2:2], "euclidean")) %>% ungroup()%>% summarize(sum(unlist(d)))
